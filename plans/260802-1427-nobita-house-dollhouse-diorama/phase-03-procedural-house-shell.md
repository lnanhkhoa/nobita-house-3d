...
| Shell + prop draw calls (32 shell + 54 props ≈ 86) sit inside the renegotiated `<120` budget | L×L | Budget raised to `<120` by the lead (`plan.md` Reconciled contracts #4) — no longer at risk. Phase 3's own gate is 34, and merging has already removed ~15 avoidable calls. `BatchedMesh` for props stays a **documented, unbuilt** Phase 9 escape hatch, triggered only if measured frame time misses the fps target, never by a draw-call count. | — |
